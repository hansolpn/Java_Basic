...
//		if (file.exists()) {
//			file.delete(); // delete 성공 시 true, 실패시 false를 리턴
...
//		}
		/*
		 - 폴더를 삭제할 때, 해당 폴더에 내부 경로가 더 존재하거나, 파일이 있는 경우에는
		   폴더를 삭제할 수 없습니다.
		 - listFiles()를 통해 파일 내부경로 또는 파일명을 얻어와서 파일부터 삭제한 후에 
		   폴더를 삭제할 수 있습니다.
		 - 지우고자 하는 경로가 여러 개라면 위의 행동을 반복에 주시면 됩니다.
		 */
		if (file.exists()) {
			if (file.isDirectory()) { // 파일이 디랙토리인지 확인
